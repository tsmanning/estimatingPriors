function [outmat] = getMinDistEst(runStruct,JSDpriorsTrCnts,priorDist,dim,domain)
...
for ii = 1:numTrCnts
    ...
    fita2(ii) = fminunc(gammaSepFxn,initVal,opts);
...
for ii = 1:numTrCnts
    ...
    switch domain
        case 'log'
            fitPrPar(ii) = interp1(log(aW(theseinds,ii)),priorPars,log(fita2(ii)));
        case 'lin'
            fitPrPar(ii) = interp1(aW(theseinds,ii),priorPars,fita2(ii));
    end
...
outmat.aW = aW;
outmat.bW = bW;
outmat.fitGamDist = fita2;
outmat.fitPrPar = fitPrPar;
