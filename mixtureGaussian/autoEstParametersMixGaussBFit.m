function [fitPars] = autoEstParametersMixGaussBFit(simDataPath,outFName,rsx,numTrials,estOpts,estDir)
...
sigfix  = 0.45*ones(1,numComp-1);
if numComp > 1
    mufix = getLogXform(linspace(0,16,numComp),0.3);
else
    mufix = getLogXform(0,0.3);
end


% Allow only 1 sigma and the weights to be fit freely
%%%% parVec = [hc sigma(1) weights];
lossFun = @(parVec)(calculate_model_nll([vfix parVec(1:numS2Conts+1) sigfix mufix parVec(numS2Conts+2:end)],ptvs_data,...
...
lb = [0.001*ones(1,numUniqCont) smallInt zeros(1,numComp)];
...
Aeq = [zeros(1,numUniqCont + 1) ones(1,numComp)];
beq = 1;
...
A = [A zeros(numUniqCont-1,numComp + 1)];
...
while indCnt <= numReps
    ...
    if isfield(initPars,'hcinit')
        ...
        parVec0         = [hcinit sigPinit(1) wPinit];
    else
...
%     wPF(indCnt,:)    = prshat(numUniqCont + 2*numComp + 1:numUniqCont + 3*numComp);
    sigPF(indCnt,:)  = [prshat(numUniqCont + 1) sigfix];
    muPF(indCnt,:)   = mufix;
    wPF(indCnt,:)    = prshat(numUniqCont + 2:end);
    
    initConds(indCnt,:) = [vfix parVec0(1:numUniqCont + 1) sigfix mufix parVec0(numUniqCont + 2:end)];
    os(indCnt)          = outStruc;
