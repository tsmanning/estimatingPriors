function [fitPars] = autoEstParametersMixGaussZeroMu(simDataPath,outFName,rsx,numTrials,estOpts,estDir)
...
lossFun = @(parVec)(calculate_model_nll(...
                    [vfix ...
                     ...
                     mufix ...
                     parVec(numS2Conts + numComp + 1:end)],ptvs_data,...
...
lb = [0.001*ones(1,numUniqCont) smallInt*ones(1,numComp) zeros(1,numComp)];

% lb = [0.001*ones(1,numUniqCont) 1e-5*ones(1,numComp) -eps*ones(1,numComp) zeros(1,numComp)];
ub = [100*ones(1,numUniqCont) 1e5*ones(1,numComp) ones(1,numComp)];               
...
A = [A zeros(numUniqCont-1,2*numComp)];
...
while indCnt <= numReps
    ...
    if isfield(initPars,'hcinit')
        ...
        parVec0         = [hcinit sigPinit wPinit];
    else
    ...
    muPF(indCnt,:)   = mufix;
    wPF(indCnt,:)    = prshat(numUniqCont + numComp + 1:end);
    
    initConds(indCnt,:) = [vfix hcinit sigPinit mufix wPinit];
    os(indCnt)          = outStruc;
