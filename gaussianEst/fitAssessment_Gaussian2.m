...
workDir = workDir(1:end-numel('LRComp.m'));
...
dataSets = {'gt_log_gauss_0.8_LR_0',   'gt_log_gauss_1_LR_0',...
            'gt_log_gauss_1.2_LR_0',   'gt_log_gauss_1.4_LR_0','gt_log_gauss_1.6_LR_0',...
            'gt_log_gauss_0.8_LR_0.05','gt_log_gauss_1_LR_0.05',...
            'gt_log_gauss_1.2_LR_0.05','gt_log_gauss_1.4_LR_0.05','gt_log_gauss_1.6_LR_0.05'};
...
parEstNames = {'parEsts_n20_collectG','parEsts_n80_collectG','parEsts_n250_collectG'};
        
trCnts      = [20 80 250];
lapseRates  = [0 0.05];
sigmas      = [0.8 1 1.2 1.4 1.6];
% trCnts      = [80];
% lapseRates  = [0];
% sigmas      = [1.4];
...
for i = 1:numDatasets
    ...
    for j = 1:numEstRuns
        ...
        runStruct(aInd,lrInd,j).sigPF      = sigPF;
        runStruct(aInd,lrInd,j).sigP       = sigP;
...
for i = 1:numLRs
    for j = 1:numSigmas
        ...
        theseFitsr   = runStruct(j,i,numRunsInd).(parFit);
        for runs = 1:numel(theseFitsr)
            theseFits(runs,:) = dNorm(getLogXform(vStim1,0.3),theseFitsr(runs),1e-14);
        end
        ...
        for h = 1:numRuns
            ...
            scatter(thisDV,theseFits(h,:),[],[0.5 0.5 1],'filled');
        ...
        if pSelect == 1
            ...
            set(gca,'ylim',[-0.5 0.5],'xlim',[vStim1(1) vStim1(end)],'xtick',vStim1,'xscale','log');
            xlabel('V_{standard}');
        elseif pSelect == 2
...
for i = 1:numLRs
    for j = 1:numSigmas
        ...
        aP = runStruct(j,i,numRunsInd).sigPF;
...
if pSDTcomp
    ...
    for i = 1:numLRs
        for j = 1:numSigmas
            ...
            [ptvsGT,~] = ...
                calculate_ptvs(vStim1,cStim1,vStim2Delta,cStim2,numtrials,sigP,gvlog,hc); 
    ...
    for i = 1:numS1Vels
        ...
        for j = 1:numS2Conts
            ...
            for sl = 1:numSigmas
                ...
                sigPF = runStruct(sl,lr2t,numRunsInd).sigPF;
                ...
                for k = 1:numResamps
                    ...
                    [~,~,biasF(k)] = ...
                        findPSENum(vStim1,vs2D,cStim2,sigPF(k,:),gvlogF(k,:),hcF(k,:),...
                        [s1C j],i,0);
